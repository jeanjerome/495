...
Il sert d’entrée à sa conception ; il ne décrit pas un comportement actuellement
implémenté par 495.
...
définitive, ni preuve de compatibilité. Aucun appel de modèle n’a été effectué
pendant cette étude.
...
- `--output-schema` contraint la réponse du modèle, mais 495 doit encore valider
  lui-même le document qu’il consomme ;
