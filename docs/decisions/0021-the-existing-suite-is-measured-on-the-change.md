...
`suite_check` evidence per iteration, in `Engine._verify` after the verifications and before
the control runs. Two readings, both pure functions in `core/suite.py`:
