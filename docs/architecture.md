...
| `coverage` | role coverage against the catalogue's roles: the roles per technology (`ROLES_BY_TECHNOLOGY`), the marker table of each technology (`PYTHON_TOOLS`, `SHELL_TOOLS`), `rows()`: one `RoleCoverage` per role with the tools whose markers hold |
