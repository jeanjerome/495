...
Two axes, and a module sits on both. **By dependency**, the packages are layers and imports point
one way (`docs/decisions/0011-package-boundaries.md`). **By effect**, inside `core`, a functional
core under an imperative shell: the directory a module sits in says what it may do to the world
(`docs/decisions/0029-a-functional-core-under-an-imperative-shell.md`). The layers, and the
purity of `core/reading/` that the second axis rests on, are import-linter contracts in
`pyproject.toml`; `lint-imports` checks them, and `tests/test_architecture.py` runs the same
check under pytest.

### The layers, by dependency

```
interfaces   cli · interactive · api · render · tui/        outermost; nothing imports it
    │
core.engine  the workflow; the only core package that builds agents
    │
...
- Inside `core`, only `engine` imports `agents` and `sandbox`. The rest of `core` (`models`,
  `reading`, `conditions`, `context`, `prompts`, `schemas`, `profile`, `coverage`, `proposals`,
  `git`, `store`, `report`, `config`, `pricing`, `budget`) depends on `core` alone.
- `core.reading` imports `core.models` and `core.reading`, and nothing else: no store, no git,
  no sandbox, no subprocess. What the harness concludes is a function of what it measured, and
  the contract is what keeps it one.
...
### The tree of `core`, by effect

```
core/
├── models/          says what things are, and nothing more          the vocabulary, under both
├── reading/         says what a measurement means, and touches nothing
├── engine/          acts: executes, persists, emits, transitions
│   ├── checks/          what is measured of a produced version
│   ├── services.py      the infrastructure a region of the engine works through
│   └── running.py       the only module of core that puts a command in the sandbox
├── profile.py · coverage.py · conditions.py · proposals.py
│                     read the project and turn it into a document
├── context.py · prompts.py · schemas.py
│                     the engine's own material: what reaches an agent, and in what shape
└── store.py · git.py · report.py · config.py · budget.py · pricing.py
                      the state on disk, the repository, and what is restituted of a run
```

Each check of the verify phase is a pair: a driver under `core/engine/checks/` that runs the
commands and writes the evidence, and a reader under `core/reading/` that says what the runs
mean and executes nothing.

| What is measured | Driver | Reader |
|---|---|---|
| the command reports the same thing twice | `engine/checks/stability.py` | `reading/verification.py::reports_the_same_twice` |
| the suite that passed on the base | `engine/checks/suite.py` | `reading/suite.py` |
| the added lines the commands execute | `engine/checks/coverage.py` | `reading/reach.py` |
| the wrong versions of the change | `engine/checks/mutation.py` | `reading/mutation.py` |
| the instrument, against its control run | `engine/checks/calibration.py` | `reading/verification.py::classify_instrument` |

### `core/models/`

A package, re-exported whole: every name is imported from `harness495.core.models`, whichever
module states it. `models/base.py` holds `StrictModel` (`extra="forbid"`), the clock and the
identifier mint; `models/enums.py` the closed sets; one module per area of the vocabulary —
`config`, `profile`, `lessons` (with `ProjectProfile`, which carries the lessons in force),
`proposals`, `retrospective`, `clarification`, `specification`, `interventions`, `evidence`,
`decisions`; and `models/run.py` the `Run` document that links them by identifier, with
`Iteration`, `Version` and `RunResult`. The modules import one another in one direction, upwards
from `base`.

### `core/reading/`

What the harness concludes from what it measured, computed and nothing else — no command, no file
of the project, no write. The eleven modules below are its whole content, and the contract on
`core.reading` is what keeps them so: a reading that needed the store, git or the sandbox would
have to leave the package first.

| Module | Responsibility |
|---|---|
| `reading/decide` | `assess()`: the pure acceptance decision over spec, evidence and reviews |
| `reading/verification` | whether a specification's verifications can carry its requirements (`assess_sufficiency`), what a pair of runs on two versions says about the instrument (`measures_the_change`, `classify_instrument`), what two runs on the same version say about the command (`reports_the_same_twice`), the failure signature and the nature of a failure (assertion or execution error), and test-file recognition (`looks_like_a_test`, read by `reading/diff.py` and `reading/suite.py` too) |
| `reading/suite` | what the change did to the test suite that passed on the base: `read_suite_changes()` reads the diff over the test files that existed there (deleted, renamed out of the runner's reach, tests removed, skips added), `count_tests()` reads the tally a runner prints, `compare_counts()` pairs the base's with the change's |
| `reading/diff` | the lines the change adds, with the number each has in the version under review, and which of them hold code a measure can be made about rather than a comment, a blank or the instrument |
| `reading/reach` | the recipe that puts the project's coverage tool in front of a command, the readers of the four report formats, and the crossing of a report with the added lines: a line held with no hit, a line the report does not hold, a file it does not instrument |
| `reading/mutation` | which line of the diff to alter and how (`plan_mutants`), and what the altered source reads, so that a verification is measured against a version of the change that is wrong in a stated way |
| `reading/catalogue` | the catalogue's recommended entries per technology and role (`RECOMMENDED`), what a test of each role must show (`ROLE_CONTRACTS`), the roles whose measure can contradict the agent (`CONTRADICTING_ROLES`), `compare()`: the profile's gaps against them, and `unmeasured_role()`: why a verification of a role cannot run in the project. `applicable()` takes the entry of a cell whose condition the profile records as having held, so no reading of the catalogue opens a file |
| `reading/retro` | the retrospective of a run: `measurements()` pairs each run of a verification on the change with its control run, `read()` turns a pair into a verdict, a contradiction, a fault or nothing, `retrospect()` states one `ToolObservation` per technology and role with the catalogue row it yields |
| `reading/lessons` | what a run showed about the project: `learn()` reads a replaced command, a correction written by hand, a blocking finding no requirement asked about, the scope a produced version stayed inside, a decision taken before the specification and a tool the retrospective found faulty; `reconcile()` keeps one record per lesson however many runs show it; `accept()`, `decline()`, `defer()` record the answer; `criteria()` adds the accepted ones to the project's criteria; `toml_lines()` and `render_document()` state them |
| `reading/stats` | `summarise()`: the runs of the store read as a series — outcomes, iterations, cost per requirement, `InstrumentStat` per command, `KindStat` per kind of verification, `PerspectiveStat` per reviewer perspective with what it found again |
| `reading/scope` | which files a change may touch, and `effective_allowed()`: the project's criteria, or the specification's where it declares none |

### `core/engine/`

| Module | Responsibility |
|---|---|
| `engine/engine.py` | the state machine: the phases, the decisions raised to the requester and how each answer moves the run, merge and integration check |
| `engine/services.py` | `RunServices`, the seven collaborators a region works through — store, sandbox, event sink, warning sink, worktree, stop check, status setter — instead of reaching for the state machine |
| `engine/checks/` | everything measured of a produced version: eight stages in an order declared as data (`checks/sequence.py`, which also holds the gate the coverage and mutation stages wait on), of which five are check groups that make no call to each other, each paired with the reader of `core/reading/` named in the table above |
| `engine/running.py` | the only module of `core` that puts a verification command in the sandbox, on the evaluated commit (`run_verification`) or on the base version (`run_control`) |
| `engine/errors.py` | `EngineError` |
| `engine/__init__.py` | what anything outside may import — `Engine` and `EngineError`, `spec_from_agent` (the reader a requester-supplied JSON specification goes through) and `worktrees_root` |

### The rest of `core`

| Module | Responsibility |
|---|---|
| `profile` | detection of languages, tooling and commands from manifests and from the tree; gathers what each technology's markers read (a `coverage.Tree`), and records on the profile which catalogue conditions held while it read them |
...
| `conditions` | the predicates that pick between the entries of one catalogue cell, and the only part of the catalogue that reads the project: a file of the tree, a dependency it declares, or a tool the role coverage names. `conditions_holding()` evaluates them once, while the project is profiled, and the profile keeps the names that held |
...
models, and `495 schema stats` publishes the shape of the series `core/reading/stats.py` reads.
