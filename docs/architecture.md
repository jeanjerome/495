...
| `verification` | running one command on the exact commit, the control run on the base version, the failure signature and the nature of a failure (assertion or execution error), sufficiency, test-file recognition |
