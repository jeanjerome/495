...
|---|---|
| `models` | a package, re-exported whole: every name is imported from `harness495.core.models`, whichever module states it. `models/base.py` holds `StrictModel` (`extra="forbid"`), the clock and the identifier mint; `models/enums.py` the closed sets; one module per area of the vocabulary — `config`, `profile`, `lessons` (with `ProjectProfile`, which carries the lessons in force), `proposals`, `retrospective`, `clarification`, `specification`, `interventions`, `evidence`, `decisions`; and `models/run.py` the `Run` document that links them by identifier, with `Iteration`, `Version` and `RunResult`. The modules import one another in one direction, upwards from `base` |
| `engine` | a package. `engine/engine.py` holds the phases, the decisions raised to the requester and how each answer moves the run, merge and integration check; `engine/services.py` is `RunServices`, the seven collaborators a region works through — store, sandbox, event sink, warning sink, worktree, stop check, status setter — instead of reaching for the state machine; `engine/errors.py` is `EngineError`; `engine/checks/` is everything measured of a produced version: eight stages in an order declared as data (`checks/sequence.py`, which also holds the gate the coverage and mutation stages wait on), of which five are check groups that make no call to each other, each paired with the pure reader of `core/` that says what its measurement means — `checks/stability.py` with `reading/verification.py::reports_the_same_twice`, `checks/suite.py` with `reading/suite.py`, `checks/coverage.py` with `reading/reach.py`, `checks/mutation.py` with `reading/mutation.py`, `checks/calibration.py` with `reading/verification.py::classify_instrument`; `engine/running.py` is the only module of `core` that puts a verification command in the sandbox, on the evaluated commit (`run_verification`) or on the base version (`run_control`). `engine/__init__.py` names what anything outside may import — `Engine` and `EngineError`, `spec_from_agent` (the reader a requester-supplied JSON specification goes through) and `worktrees_root` |
| `reading` | what the harness concludes from what it measured, computed and nothing else — no command, no file of the project, no write. The eleven modules below are its whole content, and the contract on `core.reading` is what keeps them so: a reading that needed the store, git or the sandbox would have to leave the package first |
